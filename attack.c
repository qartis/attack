...
void toggle_mute(){
  MUS(Mix_VolumeMusic(Mix_VolumeMusic(-1)==0?128:0);)
  MUS(Mix_Volume(-1, Mix_Volume(-1,-1)==0?128:0);)
}
...
void play_intro(){
  SDL_Event event;
  Uint8 *keys;
  MUS(Mix_PlayMusic(songs[INTRO_SONG], 0);)
  draw_intro_frame(0);
  for(;;){
    SDL_WaitEvent(&event);
...
void draw_castle(castle *cast){
  int i;
  for(i=0;i<12;i++) draw_object(&(cast->bricks[i]));
}
...
refresh:
  ...
  DBG(char verstring[32];
      sprintf(verstring,"%g",ver);
      SFont_Write(screen,font[1],1,(screen->h - SFont_TextHeight(font[1])),verstring);
     );
  draw_points(1);
  SDL_UpdateRect(screen,0,0,0,0);
  for (;;) {
    ...
    if (event.type == SDL_USEREVENT){
      MUS(Mix_HookMusicFinished(NULL);)
      play_intro();
      goto music;
    }
    keys = SDL_GetKeyState(NULL);
...
void toggle_pause(int *is_paused){
  if (*is_paused){
    MUS(Mix_HaltChannel(PAUSE_WAV);)
    MUS(Mix_ResumeMusic();)
    *is_paused = 0;
  } else {
    MUS(Mix_PauseMusic();)
    MUS(Mix_PlayChannel(PAUSE_WAV,sounds[PAUSE_WAV],-1);)
    *is_paused = 1;
  }
}
...
void game(int enemy_move_rate, int padding){
  ...
  MUS(Mix_HaltChannel(UFO_WAV);)// if there was a ufo onscreen when player won last round
  ...
  while (lives > 0 && !requests_quit) {
      ...
      if (keys[SDLK_0 + i] == SDL_PRESSED || keys[SDLK_KP0 + i] == SDL_PRESSED) {
        ...
        if (!waspressed[i]){
          ...
          if (-1 < (j = can_shoot(i))) {
            ...
            MUS(Mix_PlayChannel(SHOT_WAV, sounds[SHOT_WAV], 0);)
            break;
          }
#ifndef RAPIDFIRE
          waspressed[i] = 1;
    ...
    if (boss.alive){
      ...
      if (boss.x > SCREEN_WIDTH || boss.x < -boss.image->w){
        ...
        else MUS(Mix_HaltChannel(UFO_WAV);)
      }
    } else if (boss_timer < 1 DBG(|| keys[SDLK_b] == SDL_PRESSED)){
      boss.alive = 1;
      MUS(Mix_PlayChannel(UFO_WAV,sounds[UFO_WAV],-1);)
      boss_timer = rand_between(BOSS_TIMER_MIN,BOSS_TIMER_MAX);
    } else {
      boss_timer--;
    }
    ...
    for (j = 0; j < MAX_SHOTS; ++j) {
      for (i = 0; i < MAX_ENEMIES; ++i) {
        if (shots[j].alive && enemies[i].alive
          ...
          MUS(Mix_PlayChannel(EXPLODE_WAV, sounds[EXPLODE_WAV], 0);)
          shots[j].alive = 0;
...
jmp:
      ...
      if (box_collision(&shots[j],&boss,padding)){
        ...
        boss.x = -boss.image->w;
        MUS(Mix_PlayChannel(UFO_EXPLODE_WAV, sounds[UFO_EXPLODE_WAV], 0);)
        MUS(Mix_HaltChannel(UFO_WAV);)
        shots[j].alive = 0;
      ...
      for(i=0;i<MAX_ENEMY_SHOTS;i++){
        if (enemy_shots[i].alive && box_collision(&shots[j],&enemy_shots[i],padding)){
          ...
          if (enemy_shots[i].alive < 1){
            ...
            explosions[MAX_ENEMIES+1+i].image = tint(explosions[MAX_ENEMIES+1+i].image,colours[WHITE]);
            enemy_shots[i].alive = 0;
          }
          break;
        }
...
void vidmode(void){
  ...
  if (NULL == (screen = SDL_SetVideoMode(SCREEN_WIDTH, SCREEN_HEIGHT, screen?screen->format->BitsPerPixel:32, flags))) {
...
;
  ...
  if ((rc = pthread_create(&data_load_thread,NULL,(void*(*)(void*))load_data,NULL))){
    DEBUG("error creating data loader thread: %s",strerror(rc));
    load_data();
  }
  MUS(Mix_Volume(UFO_WAV,MIX_MAX_VOLUME/10);)
  MUS(Mix_Volume(EXPLODE_WAV,MIX_MAX_VOLUME/4);)
  MUS(Mix_Volume(UFO_EXPLODE_WAV,MIX_MAX_VOLUME/4);)
  ...
  while (!requests_quit) {
    ...
    if ((rc = pthread_create(&patch_ask_thread,NULL,(void*(*)(void*))ask_for_patch,NULL))){
    ...
    for(current_player=0;current_player<num_players&&!requests_quit;++current_player){
      ...
      if (current_player == 0){
        MUS(int id = LEVEL1_SONG + rand()%(LEVEL3_SONG-LEVEL1_SONG);
      ...
      do {
        ...
        MUS(Mix_HaltChannel(UFO_WAV);)// if there was a ufo onscreen when player died
        enemy_move_rate -= MOVE_RATE_DECREMENT;
        enemy_move_rate = MAX(enemy_move_rate,MINIMUM_MOVE_RATE);
    ...
    if (requests_quit) break;
    SDL_Delay(1000);
    MUS(Mix_PlayMusic(songs[DEAD_SONG],-1);)
    for (i = 0; i < num_players; i++) get_name(i);
    if (requests_quit) break;
    highscores();
    MUS(Mix_FadeOutMusic(400);)
    SDL_Delay(400);
  }
