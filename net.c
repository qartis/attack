...
size_t curl_got_highscore_data(void* buffer, size_t size, size_t nmemb, void* userp){
  ...
  *(((char*)buffer)+nmemb) = '\0';
  ...
  DBG(for(i=0;i<10;i++){
    DEBUG("score %d: (%6d %c%c%c)",i+1,high_scores[i].points,high_scores[i].name[0],high_scores[i].name[1],
...
void highscores(){
  ...
  sprintf(data,"n=%d&n1=%c%c%c&s1=%d&n2=%c%c%c&s2=%d&v=%lf&e=%c",num_players,
      player_scores[0].name[0],player_scores[0].name[1],player_scores[0].name[2],player_scores[0].points,
      player_scores[1].name[0],player_scores[1].name[1],player_scores[1].name[2],player_scores[1].points,ver,edition);
...
refresh:
  ...
  if (high_score_data_is_real){
    ...
    if (0 != stat(".patch",NULL)){
      SFont_Write(screen,font[2],(SCREEN_WIDTH-SFont_TextWidth(font[0],"UPDATE AVAILABLE"))/2,450,"UPDATE AVAILABLE");
      SFont_Write(screen,font[2],(SCREEN_WIDTH-SFont_TextWidth(font[0],"PLEASE RESTART"))/2,460,"PLEASE RESTART");
    }
  } else {
...
void ask_for_patch(void){
  CURLcode res;
  DEBUG("asking for patch");
  FILE *patch_file = fopen(".pratch","wb");
  if (patch_file == NULL){
    DEBUG("Unable to make patch file, aborting patch");
    return;
  }
  char data[64];
  sprintf(data,"v=%g&e=%c",ver,edition);
  CURL *easy = curl_easy_init();
  curl_easy_setopt(easy, CURLOPT_URL, "http://h.qartis.com/attack/version.php");
  curl_easy_setopt(easy, CURLOPT_POSTFIELDS, data);
  curl_easy_setopt(easy, CURLOPT_WRITEDATA, patch_file);
  curl_easy_setopt(easy, CURLOPT_CONNECTTIMEOUT, 10L);
  curl_easy_setopt(easy, CURLOPT_NOPROGRESS, 1L);
  res = curl_easy_perform(easy);
  if(CURLE_OK == res) {
    long resp;
    res = curl_easy_getinfo(easy, CURLINFO_RESPONSE_CODE, &resp);
   
    if(CURLE_OK == res && resp == 200L){
      DEBUG("got patch: a");
    } else {
      DEBUG("error downloading patch");
      remove(".patch");
    }
  } else {
    DEBUG("curl error (%d): %s",res,curl_easy_strerror(res));
    remove(".patch");
  }
